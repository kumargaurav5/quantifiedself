...
def view_tracker(record_id):
    ...
    try:
        import sqlite3
        con = sqlite3.connect('myProjectApp/data.sqlite')
        ...
        plt.ylabel('Values')
        plt.tight_layout()
        plt.savefig('myProjectApp/static/Images/graph.png')
        # plt.show()

        gon = sqlite3.connect('myProjectApp/data.sqlite')
        g = gon.cursor()
...
def view_tracker(record_id):
    ...
    try:
        import sqlite3
        con = sqlite3.connect('F:\MAD1\myProjectApp\data.sqlite')
        print("Database opened successfully")
        c = con.cursor()
        c.execute('SELECT timestamp, value FROM Log WHERE user_id={} AND tracker_id={}'.format(current_user.id,
                                                                                               selected_tracker.id))
        ...
        plt.ylabel('Values')
        plt.tight_layout()
        plt.savefig('F:/MAD1/myProjectApp/static/Images/graph.png')
        # plt.show()

        gon = sqlite3.connect('F:/MAD1/myProjectApp/data.sqlite')
        g = gon.cursor()
        added_date_time = g.execute('SELECT added_date_time FROM Log WHERE '
                                    'id=(SELECT max(id) FROM Log WHERE tracker_id={})'.format(record_id))
